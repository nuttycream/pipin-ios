...
struct GPIOPinsView: View {
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 10) {
                ForEach(["Setup", "Reset", "Terminate"], id: \.self) { label in
                    Button(action: {
                        // TODO: Action for Setup, Reset, Terminate
                    }) {
                        Text(label)
                            .foregroundColor(.white)
                            .padding()
            ...
            LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible())], spacing: 8) {
                ForEach(gpioPins, id: \.self) { pin in
                    Button(action: {
...
struct QueueView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            Text("[00:15:00] failed to toggle gpio 23: GPIO Not Initialized")
                .foregroundColor(.red)
                .font(.system(size: 14, design: .monospaced))
        }
    }
